...
if ! command -v conda &> /dev/null; then
    # Check if conda exists in common locations
    if [ -f "$HOME/miniconda3/bin/conda" ]; then
        echo "Conda found in $HOME/miniconda3 but not in PATH. Initializing..."
        eval "$($HOME/miniconda3/bin/conda shell.bash hook)"
    elif [ -f "$HOME/anaconda3/bin/conda" ]; then
        echo "Conda found in $HOME/anaconda3 but not in PATH. Initializing..."
        ...
        # Install Miniconda with automatic yes to all prompts
