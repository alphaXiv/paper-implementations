...
# Parent directory (rope_pp)
ROPE_PP_DIR="$( cd "$SCRIPT_DIR/.." && pwd )"

# Install uv (if not already installed)
command -v uv &> /dev/null || curl -LsSf https://astral.sh/uv/install.sh | sh

# Create a .venv local virtual environment in the rope_pp directory (if it doesn't exist)
cd "$ROPE_PP_DIR"
...
# Change back to visualizer directory
cd "$SCRIPT_DIR"
...
if [ -f "sample-wiki-point.txt" ]; then
    # Load full text and truncate to ~10,000 tokens (~50KB of text)
    TEXT=$(head -c 50000 sample-wiki-point.txt)
    echo "Using text from sample-wiki-point.txt (truncated to ~10,000 tokens for memory)"
else
    TEXT="${TEXT:-The quick brown fox jumps over the lazy dog and runs through the forest.}"
...
echo "  python visualize_attention.py --model-path SII-xrliu/RoPEPP_EH-DCLM-376M-4k \\"
