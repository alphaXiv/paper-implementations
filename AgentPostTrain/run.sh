...
if [ ! -f "containers/standard_minimal.sif" ]; then
    ...
    sudo apptainer build containers/standard_minimal.sif containers/standard_minimal.def
fi
...
if [ ! -d "$MODEL_CACHE" ] || [ -z "$(ls -A "$MODEL_CACHE" 2>/dev/null)" ]; then
    ...
    sudo apptainer exec --nv --bind "${HF_HOME}:${HF_HOME}" --env HF_HOME="${HF_HOME}" \
        containers/standard_minimal.sif python3 -c "
...
PROMPT=$(sed "s|\${MODEL}|${MODEL}|g; s|\${HOURS}|${HOURS}|g" prompt.txt)
...
cp eval/evaluate.py "$JOB_DIR/task/"
cp /tmp/timer.sh "$JOB_DIR/task/timer.sh"
echo "$PROMPT" > "$JOB_DIR/task/prompt.txt"
cp agents/codex/solve.sh "$JOB_DIR/agent_solve.sh"
...
sudo apptainer exec --nv -c \
    ...
    --pwd "/home/ben/task" \
    --writable-tmpfs \
    containers/standard_minimal.sif \
