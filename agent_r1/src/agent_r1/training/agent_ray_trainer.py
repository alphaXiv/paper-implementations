    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                status = "WARMUP" if not actor_updated else "TRAINING"
                progress_bar.set_postfix({
                    "status": status,
                    "loss": f"{loss:.4f}" if actor_updated else "N/A",
                    "grad_norm": f"{grad_norm:.3f}" if actor_updated else "N/A",
                    "lr": f"{lr:.6f}",
                ...
                print(f"Step {self.global_steps:05d} | Status: {status} | Loss: {loss:.6f if actor_updated else 'N/A'} | Grad Norm: {grad_norm:.4f if actor_updated else 'N/A'} | LR: {lr:.6f} | Rewards: {rewards_mean:.4f} | Adv: {advantages_mean:.4f} | Step Time: {step_time:.2f}s | Tok/sec: {tokens_per_sec:.0f} | MFU: {mfu:.2f}")
