...
def process_args(args):
    ...
    parser.add_argument('--ocr-type', type=str, default='deepseek', help='OCR type (deepseek, olmocr2, chandra, etc.)')
    parser.add_argument('--language', type=str, default='all', help='Language filter (all, english, simplified_chinese)')
...
if __name__ == '__main__':
    ...
    for task in cfg.keys():
        ...
        if cfg[task]['dataset']['prediction'].get('data_path'):
            base_name = os.path.basename(cfg[task]['dataset']['prediction']['data_path'])
            match_method = cfg[task]['dataset'].get('match_method', 'quick_match')
            # Construct naming convention: ocr_type_language_match_method
            save_name = f"{ocr_type}_{language}_{match_method}"
        else:
